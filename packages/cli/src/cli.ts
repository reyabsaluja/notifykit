...
async function handleServe(args: string[]): Promise<number> {
  ...
  return runServe({
    ...
    port,
    devUser: values["dev-user"] as string,
    basePath: values["base-path"],
  });
}

async function handleSend(args: string[]): Promise<number> {
  const { values } = parseArgs({
    args,
    options: {
      config: { type: "string" },
  ...
  if (!values.to) throw new Error("--to <recipientId> is required");
  if (!values.id) throw new Error("--id <notificationId> is required");

  let payload: Record<string, unknown>;
  try {
    const raw = JSON.parse(values.payload ?? "{}") as unknown;
