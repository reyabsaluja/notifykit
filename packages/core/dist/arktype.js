 ...
 */
export function arktypePayload(arkType) {
    const payload = {};
    for (const prop of arkType.props) {
        ...
        if (expr in expressionToPrimitive) {
            ...
            console.warn(`[notifykit/arktype] Field "${key}" (expression: "${expr}") was not mapped ` +
    ...
    return {
        payload,
        validate: (data) => arkType.assert(data),
    };
}
